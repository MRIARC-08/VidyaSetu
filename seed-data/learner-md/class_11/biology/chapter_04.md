# Chapter 4 - Animal Kingdom

Class 11 Biology

## Chapter Profile

| Item         | Details                                                                                      |
| ------------ | -------------------------------------------------------------------------------------------- |
| Class        | 11                                                                                           |
| Subject      | Biology                                                                                      |
| Chapter      | 4                                                                                            |
| Title        | Animal Kingdom                                                                               |
| Main theme   | Classification of animals using fundamental structural and developmental features            |
| Major groups | Non-chordates and chordates                                                                  |
| Core skill   | Identify phyla and classes from diagnostic characters                                        |
| Learner goal | Build a reliable comparison map of animal groups instead of memorising disconnected examples |

## How To Study This Chapter

Animal Kingdom is a comparison-heavy chapter. The best method is to first master the classification criteria, then use those criteria to identify each phylum.

Do not begin by memorising examples. Start with these questions:

1. What is the level of organisation?
2. What type of symmetry is present?
3. Is the animal diploblastic or triploblastic?
4. Is the coelom absent, false, or true?
5. Is segmentation present?
6. Is a notochord present?
7. What special structure makes the group identifiable?

Once these questions are clear, the phyla become much easier to separate.

## Learning Outcomes

By the end of this chapter, you should be able to:

- explain why animals must be classified using fundamental features;
- distinguish cellular, tissue, organ, and organ-system levels of organisation;
- compare asymmetry, radial symmetry, and bilateral symmetry;
- distinguish diploblastic and triploblastic animals;
- compare acoelomate, pseudocoelomate, and coelomate animals;
- explain metameric segmentation;
- define notochord and use it to distinguish chordates from non-chordates;
- describe the major features of Porifera, Cnidaria, Ctenophora, Platyhelminthes, Aschelminthes, Annelida, Arthropoda, Mollusca, Echinodermata, Hemichordata, and Chordata;
- compare chordates and non-chordates;
- describe protochordates and vertebrates;
- compare major vertebrate classes;
- answer reasoning questions about air bladder, coelom, parasitism, flight adaptations, and vertebrate identity.

## Key Terms

| Term                  | Meaning                                                                   |
| --------------------- | ------------------------------------------------------------------------- |
| Animalia              | Kingdom of multicellular, heterotrophic eukaryotes without cell walls     |
| Level of organisation | Degree to which cells are arranged into tissues, organs, and systems      |
| Cellular level        | Cells are loosely arranged without true tissues                           |
| Tissue level          | Similar cells form tissues                                                |
| Organ level           | Tissues form organs                                                       |
| Organ-system level    | Organs form functional systems                                            |
| Symmetry              | Arrangement of body parts around an axis or plane                         |
| Asymmetry             | No plane divides body into equal halves                                   |
| Radial symmetry       | Any plane through central axis gives equal halves                         |
| Bilateral symmetry    | Only one plane divides body into left and right halves                    |
| Diploblastic          | Body develops from two germ layers: ectoderm and endoderm                 |
| Triploblastic         | Body develops from three germ layers: ectoderm, mesoderm, endoderm        |
| Coelom                | Body cavity lined by mesoderm                                             |
| Acoelomate            | Animal without body cavity                                                |
| Pseudocoelomate       | Animal with body cavity not completely lined by mesoderm                  |
| Coelomate             | Animal with true coelom                                                   |
| Metamerism            | Serial repetition of body segments                                        |
| Notochord             | Mesodermal rod-like structure on dorsal side during embryonic development |
| Chordate              | Animal with notochord at least during some stage                          |
| Non-chordate          | Animal without notochord                                                  |
| Choanocyte            | Collar cell in sponges                                                    |
| Cnidoblast            | Stinging cell of cnidarians                                               |
| Comb plate            | Ciliated plate used in movement by ctenophores                            |
| Flame cell            | Excretory and osmoregulatory cell in flatworms                            |
| Parapodia             | Lateral appendages in some annelids                                       |
| Radula                | File-like feeding organ of molluscs                                       |
| Water vascular system | Echinoderm system used in locomotion, feeding, and respiration            |
| Stomochord            | Structure in hemichordates similar to a notochord                         |
| Operculum             | Gill cover of bony fishes                                                 |
| Air bladder           | Buoyancy-regulating organ in bony fishes                                  |
| Poikilotherm          | Cold-blooded animal with body temperature varying with environment        |
| Homoiotherm           | Warm-blooded animal maintaining constant body temperature                 |
| Oviparous             | Egg-laying                                                                |
| Viviparous            | Giving birth to young ones                                                |

## Chapter Map

| Unit | Topic                          | What To Master                                                       |
| ---- | ------------------------------ | -------------------------------------------------------------------- |
| 4.1  | Need for animal classification | Why common features matter                                           |
| 4.2  | Basis of classification        | Organisation, symmetry, germ layers, coelom, segmentation, notochord |
| 4.3  | Non-chordate phyla             | Porifera to Hemichordata                                             |
| 4.4  | Chordata                       | Chordate characters and subphyla                                     |
| 4.5  | Vertebrates                    | Cyclostomata, fishes, amphibians, reptiles, birds, mammals           |
| 4.6  | Exercise reasoning             | Coelom, parasitism, air bladder, flight, oviparity, viviparity       |

## 1. Why Animal Classification Is Needed

More than a million animal species have been described. Animals vary greatly in form, body plan, habitat, nutrition, movement, and reproduction. Without classification, it would be difficult to identify newly discovered animals or understand their relationships.

Classification helps by:

- arranging animal diversity into meaningful groups;
- identifying new species systematically;
- comparing body plans;
- understanding evolutionary relationships;
- predicting features from group identity;
- reducing confusion caused by superficial similarity.

For example, a worm-like body alone is not enough for classification. A flatworm, roundworm, annelid, and hemichordate can all look worm-like, but they differ in coelom, segmentation, organ systems, and developmental characters.

## 2. Basis Of Animal Classification

Animal classification uses fundamental features such as:

- level of organisation;
- symmetry;
- germ layers;
- coelom;
- segmentation;
- notochord;
- digestive system;
- circulatory system;
- respiratory system;
- reproductive pattern;
- special diagnostic structures.

## 3. Levels Of Organisation

Although all animals are multicellular, they do not all show the same organisation of cells.

## 3.1 Cellular Level

At the cellular level, cells are arranged as loose aggregates. True tissues are absent.

Example: Porifera.

Sponges show some division of labour among cells, but their cells are not organised into true tissues.

## 3.2 Tissue Level

At the tissue level, similar cells performing the same function are arranged into tissues.

Example: Cnidaria.

## 3.3 Organ Level

At the organ level, tissues combine to form organs specialised for particular functions.

Example: Platyhelminthes.

## 3.4 Organ-System Level

At the organ-system level, organs work together as functional systems.

Examples include annelids, arthropods, molluscs, echinoderms, and chordates.

## 3.5 Comparison Of Organisation Levels

| Level        | Meaning                     | Example Group                  |
| ------------ | --------------------------- | ------------------------------ |
| Cellular     | Cells form loose aggregates | Porifera                       |
| Tissue       | Cells form tissues          | Cnidaria, Ctenophora           |
| Organ        | Tissues form organs         | Platyhelminthes                |
| Organ-system | Organs form systems         | Annelida onward in many groups |

## 4. Digestive And Circulatory Patterns

## 4.1 Digestive System

| Type                        | Description                             |
| --------------------------- | --------------------------------------- |
| Incomplete digestive system | One opening acts as both mouth and anus |
| Complete digestive system   | Separate mouth and anus are present     |

An incomplete digestive system is seen in groups such as cnidarians and flatworms. A complete digestive system is seen in many higher groups.

## 4.2 Circulatory System

| Type                      | Description                                                          |
| ------------------------- | -------------------------------------------------------------------- |
| Open circulatory system   | Blood is pumped out of the heart and directly bathes tissues         |
| Closed circulatory system | Blood flows through vessels such as arteries, veins, and capillaries |

Open circulation is found in arthropods and molluscs. Closed circulation is found in annelids and chordates.

## 5. Symmetry

Symmetry refers to how body parts are arranged.

## 5.1 Asymmetry

An animal is asymmetrical if no plane passing through the centre divides it into equal halves.

Example: many sponges.

## 5.2 Radial Symmetry

An animal is radially symmetrical if any plane passing through the central axis divides the body into two equal halves.

Examples:

- cnidarians;
- ctenophores;
- adult echinoderms.

Radial symmetry is useful in animals that are sessile, slow-moving, or interact with the environment from many directions.

## 5.3 Bilateral Symmetry

An animal is bilaterally symmetrical if only one plane divides it into equal left and right halves.

Examples:

- annelids;
- arthropods;
- molluscs;
- chordates.

Bilateral symmetry is usually linked with directed movement and development of a head region.

## 6. Germ Layers

During embryonic development, body tissues arise from germ layers.

## 6.1 Diploblastic Animals

Diploblastic animals have two germ layers:

- ectoderm;
- endoderm.

A jelly-like layer called mesoglea may be present between them.

Example: cnidarians.

## 6.2 Triploblastic Animals

Triploblastic animals have three germ layers:

- ectoderm;
- mesoderm;
- endoderm.

Animals from Platyhelminthes to Chordata are triploblastic.

## 7. Coelom

The presence, absence, or type of body cavity is important in classification.

## 7.1 Acoelomate

Acoelomates lack a body cavity.

Example: Platyhelminthes.

## 7.2 Pseudocoelomate

Pseudocoelomates have a body cavity, but it is not completely lined by mesoderm.

Example: Aschelminthes.

## 7.3 Coelomate

Coelomates have a true body cavity lined by mesoderm.

Examples:

- annelids;
- molluscs;
- arthropods;
- echinoderms;
- hemichordates;
- chordates.

## 7.4 Why Coelom Matters

A true coelom provides space for internal organs, allows organ systems to develop more complexity, and helps separate the body wall from the gut wall. It is therefore a major feature in animal classification.

## 8. Segmentation

Segmentation means division of the body into repeated parts.

Metameric segmentation means external and internal segmentation with serial repetition of at least some organs.

Example: earthworm.

Segmentation first becomes clearly seen in Annelida.

## 9. Notochord

Notochord is a mesodermally derived rod-like structure formed on the dorsal side during embryonic development in some animals.

Animals with a notochord are called chordates.

Animals without a notochord are called non-chordates.

Non-chordates include Porifera to Echinodermata, and Hemichordata is treated separately under non-chordates in this chapter.

## 10. Quick Classification Logic

| Feature Asked         | If Present                                      | If Absent Or Different               |
| --------------------- | ----------------------------------------------- | ------------------------------------ |
| Notochord             | Chordata                                        | Non-chordate groups                  |
| True coelom           | Annelida and higher coelomate groups            | Acoelomate or pseudocoelomate groups |
| Metamerism            | Annelida and Arthropoda show clear segmentation | Many lower groups lack it            |
| Cnidoblasts           | Cnidaria                                        | Other phyla                          |
| Comb plates           | Ctenophora                                      | Other phyla                          |
| Jointed appendages    | Arthropoda                                      | Other phyla                          |
| Radula                | Mollusca                                        | Other phyla                          |
| Water vascular system | Echinodermata                                   | Other phyla                          |

## 11. Phylum Porifera

Members of Porifera are commonly called sponges.

## 11.1 Main Features Of Porifera

| Feature       | Description                    |
| ------------- | ------------------------------ |
| Common name   | Sponges                        |
| Habitat       | Mostly marine; some freshwater |
| Symmetry      | Mostly asymmetrical            |
| Organisation  | Cellular level                 |
| Body pores    | Present                        |
| Canal system  | Present                        |
| Digestion     | Intracellular                  |
| Skeleton      | Spicules or spongin fibres     |
| Sexes         | Not separate in most forms     |
| Fertilisation | Internal                       |
| Development   | Indirect with larval stage     |

## 11.2 Canal System In Sponges

Water enters through small pores called ostia.

It moves into a central cavity called spongocoel.

It exits through a larger opening called osculum.

This water transport helps in:

- food collection;
- respiratory exchange;
- waste removal.

## 11.3 Choanocytes

Choanocytes are collar cells that line the spongocoel and canals.

They help generate water current and capture food particles.

## 11.4 Reproduction In Porifera

Sponges reproduce:

- asexually by fragmentation;
- sexually by formation of gametes.

Most sponges are hermaphrodites, meaning eggs and sperms may be produced by the same individual.

## 11.5 Examples Of Porifera

Examples:

- Sycon;
- Spongilla;
- Euspongia.

## 12. Phylum Coelenterata Or Cnidaria

Cnidarians are aquatic animals. Most are marine.

## 12.1 Main Features Of Cnidaria

| Feature          | Description                     |
| ---------------- | ------------------------------- |
| Habitat          | Aquatic, mostly marine          |
| Habit            | Sessile or free-swimming        |
| Symmetry         | Radial                          |
| Organisation     | Tissue level                    |
| Germ layers      | Diploblastic                    |
| Special cells    | Cnidoblasts or cnidocytes       |
| Digestive cavity | Gastrovascular cavity           |
| Digestive system | Incomplete                      |
| Digestion        | Extracellular and intracellular |
| Skeleton         | Calcium carbonate in corals     |

## 12.2 Cnidoblasts

Cnidoblasts contain stinging capsules called nematocysts.

They are used for:

- anchorage;
- defence;
- capture of prey.

## 12.3 Polyp And Medusa

Cnidarians may show two body forms.

| Form   | Description                       | Example        |
| ------ | --------------------------------- | -------------- |
| Polyp  | Sessile and cylindrical           | Hydra, Adamsia |
| Medusa | Umbrella-shaped and free-swimming | Aurelia        |

## 12.4 Metagenesis

Some cnidarians show alternation of generations called metagenesis.

In such forms:

- polyps produce medusae asexually;
- medusae produce polyps sexually.

Example: Obelia.

## 12.5 Examples Of Cnidaria

Examples:

- Physalia;
- Adamsia;
- Pennatula;
- Gorgonia;
- Meandrina;
- Aurelia;
- Hydra.

## 13. Phylum Ctenophora

Ctenophores are commonly called sea walnuts or comb jellies.

## 13.1 Main Features Of Ctenophora

| Feature         | Description                        |
| --------------- | ---------------------------------- |
| Habitat         | Exclusively marine                 |
| Symmetry        | Radial                             |
| Germ layers     | Diploblastic                       |
| Organisation    | Tissue level                       |
| Locomotion      | Eight rows of ciliated comb plates |
| Digestion       | Extracellular and intracellular    |
| Bioluminescence | Well marked                        |
| Sexes           | Not separate                       |
| Reproduction    | Sexual only                        |
| Fertilisation   | External                           |
| Development     | Indirect                           |

## 13.2 Examples Of Ctenophora

Examples:

- Pleurobrachia;
- Ctenoplana.

## 14. Phylum Platyhelminthes

Platyhelminthes are flatworms.

## 14.1 Main Features Of Platyhelminthes

| Feature          | Description                 |
| ---------------- | --------------------------- |
| Body shape       | Dorsoventrally flattened    |
| Habit            | Many are endoparasites      |
| Symmetry         | Bilateral                   |
| Germ layers      | Triploblastic               |
| Coelom           | Acoelomate                  |
| Organisation     | Organ level                 |
| Digestive system | Incomplete in many forms    |
| Excretion        | Flame cells                 |
| Sexes            | Usually not separate        |
| Fertilisation    | Internal                    |
| Development      | Often through larval stages |

## 14.2 Parasitic Adaptations

Parasitic flatworms may show:

- hooks;
- suckers;
- flattened body;
- high reproductive capacity;
- nutrient absorption through body surface;
- complex larval stages.

## 14.3 Flame Cells

Flame cells help in:

- osmoregulation;
- excretion.

## 14.4 Regeneration

Planaria has high regeneration capacity.

## 14.5 Examples Of Platyhelminthes

Examples:

- Taenia;
- Fasciola;
- Planaria.

## 15. Phylum Aschelminthes

Aschelminthes are roundworms.

## 15.1 Main Features Of Aschelminthes

| Feature          | Description                                 |
| ---------------- | ------------------------------------------- |
| Body shape       | Circular in cross-section                   |
| Habit            | Free-living or parasitic                    |
| Habitat          | Aquatic, terrestrial, plant or animal hosts |
| Organisation     | Organ-system level                          |
| Symmetry         | Bilateral                                   |
| Germ layers      | Triploblastic                               |
| Coelom           | Pseudocoelomate                             |
| Digestive system | Complete                                    |
| Pharynx          | Muscular and well developed                 |
| Excretion        | Excretory tube                              |
| Sexes            | Separate                                    |
| Fertilisation    | Internal                                    |
| Development      | Direct or indirect                          |

## 15.2 Male And Female Difference

Aschelminthes are dioecious. Males and females are distinct.

Females are often longer than males.

## 15.3 Examples Of Aschelminthes

Examples:

- Ascaris;
- Wuchereria;
- Ancylostoma.

## 16. Phylum Annelida

Annelids are segmented worms.

## 16.1 Main Features Of Annelida

| Feature        | Description                                  |
| -------------- | -------------------------------------------- |
| Habitat        | Marine, freshwater, or terrestrial           |
| Habit          | Free-living or sometimes parasitic           |
| Organisation   | Organ-system level                           |
| Symmetry       | Bilateral                                    |
| Germ layers    | Triploblastic                                |
| Coelom         | True coelom                                  |
| Segmentation   | Metameric segmentation                       |
| Locomotion     | Longitudinal and circular muscles            |
| Circulation    | Closed                                       |
| Excretion      | Nephridia                                    |
| Nervous system | Paired ganglia and double ventral nerve cord |
| Reproduction   | Sexual                                       |

## 16.2 Metamerism

Annelids show repeated body segments called metameres.

This segmentation is both external and internal.

## 16.3 Parapodia

Aquatic annelids such as Nereis have lateral appendages called parapodia.

Parapodia help in swimming.

## 16.4 Sexes In Annelids

| Animal    | Sex Condition |
| --------- | ------------- |
| Nereis    | Dioecious     |
| Earthworm | Monoecious    |
| Leech     | Monoecious    |

## 16.5 Examples Of Annelida

Examples:

- Nereis;
- Pheretima;
- Hirudinaria.

## 17. Phylum Arthropoda

Arthropoda is the largest phylum of Animalia.

It includes insects and many other forms. More than two-thirds of all named animal species are arthropods.

## 17.1 Main Features Of Arthropoda

| Feature       | Description                         |
| ------------- | ----------------------------------- |
| Organisation  | Organ-system level                  |
| Symmetry      | Bilateral                           |
| Germ layers   | Triploblastic                       |
| Segmentation  | Present                             |
| Coelom        | Coelomate                           |
| Body covering | Chitinous exoskeleton               |
| Body regions  | Head, thorax, abdomen in many forms |
| Appendages    | Jointed                             |
| Circulation   | Open                                |
| Excretion     | Malpighian tubules                  |
| Sexes         | Mostly separate                     |
| Fertilisation | Usually internal                    |
| Development   | Direct or indirect                  |

## 17.2 Respiratory Organs In Arthropods

Arthropods may respire through:

- gills;
- book gills;
- book lungs;
- tracheal system.

## 17.3 Sensory Organs

Arthropods may have:

- antennae;
- compound eyes;
- simple eyes;
- statocysts for balance.

## 17.4 Why Arthropods Are So Successful

Important reasons include:

- chitinous exoskeleton for protection and water conservation;
- jointed appendages for diverse movement and feeding;
- body segmentation and specialisation;
- wide range of respiratory organs;
- well-developed sensory organs;
- internal fertilisation in many forms;
- high reproductive potential;
- metamorphosis in many insects;
- ability to occupy land, water, and air habitats.

## 17.5 Examples Of Arthropoda

| Category                       | Examples                   |
| ------------------------------ | -------------------------- |
| Economically important insects | Apis, Bombyx, Laccifer     |
| Disease vectors                | Anopheles, Culex, Aedes    |
| Gregarious pest                | Locusta                    |
| Living fossil                  | Limulus                    |
| Other examples                 | Prawn, scorpion, butterfly |

## 18. Phylum Mollusca

Mollusca is the second largest animal phylum.

## 18.1 Main Features Of Mollusca

| Feature       | Description                        |
| ------------- | ---------------------------------- |
| Habitat       | Terrestrial or aquatic             |
| Organisation  | Organ-system level                 |
| Symmetry      | Bilateral                          |
| Germ layers   | Triploblastic                      |
| Coelom        | Coelomate                          |
| Body          | Soft and unsegmented               |
| Shell         | Usually calcareous                 |
| Body regions  | Head, muscular foot, visceral hump |
| Mantle        | Soft layer over visceral hump      |
| Mantle cavity | Space containing gills             |
| Feeding organ | Radula                             |
| Sexes         | Usually separate                   |
| Development   | Indirect                           |

## 18.2 Mantle And Mantle Cavity

The mantle is a soft layer of skin over the visceral hump.

The space between mantle and visceral hump is the mantle cavity.

The mantle cavity contains feather-like gills that help in respiration and excretion.

## 18.3 Radula

Radula is a file-like rasping organ used in feeding.

It is a key feature of molluscs.

## 18.4 Examples Of Mollusca

Examples:

- Pila;
- Pinctada;
- Sepia;
- Loligo;
- Octopus;
- Aplysia;
- Dentalium;
- Chaetopleura.

## 19. Phylum Echinodermata

Echinoderms are marine animals with spiny bodies.

## 19.1 Main Features Of Echinodermata

| Feature          | Description                         |
| ---------------- | ----------------------------------- |
| Habitat          | Marine                              |
| Organisation     | Organ-system level                  |
| Skeleton         | Endoskeleton of calcareous ossicles |
| Adult symmetry   | Radial                              |
| Larval symmetry  | Bilateral                           |
| Germ layers      | Triploblastic                       |
| Coelom           | Coelomate                           |
| Digestive system | Complete                            |
| Mouth            | Ventral side                        |
| Anus             | Dorsal side                         |
| Special system   | Water vascular system               |
| Excretory system | Absent                              |
| Sexes            | Separate                            |
| Fertilisation    | Usually external                    |
| Development      | Indirect with free-swimming larva   |

## 19.2 Water Vascular System

The water vascular system helps in:

- locomotion;
- capture of food;
- transport of food;
- respiration.

This is the most distinctive feature of echinoderms.

## 19.3 Examples Of Echinodermata

Examples:

- Asterias;
- Echinus;
- Antedon;
- Cucumaria;
- Ophiura.

## 20. Phylum Hemichordata

Hemichordata was earlier considered under Chordata, but it is now treated as a separate non-chordate phylum in this chapter.

## 20.1 Main Features Of Hemichordata

| Feature       | Description              |
| ------------- | ------------------------ |
| Habitat       | Marine                   |
| Body form     | Worm-like                |
| Organisation  | Organ-system level       |
| Symmetry      | Bilateral                |
| Germ layers   | Triploblastic            |
| Coelom        | Coelomate                |
| Body regions  | Proboscis, collar, trunk |
| Circulation   | Open                     |
| Respiration   | Gills                    |
| Excretion     | Proboscis gland          |
| Sexes         | Separate                 |
| Fertilisation | External                 |
| Development   | Indirect                 |

## 20.2 Stomochord

Hemichordates have a structure called stomochord in the collar region.

It is similar to a notochord but is not the same as the true chordate notochord.

## 20.3 Examples Of Hemichordata

Examples:

- Balanoglossus;
- Saccoglossus.

## 21. Phylum Chordata

Chordates are animals that show a notochord at least during some stage of development.

## 21.1 Fundamental Features Of Chordates

Chordates have:

- notochord;
- dorsal hollow nerve cord;
- paired pharyngeal gill slits;
- post-anal tail;
- closed circulatory system.

They are bilaterally symmetrical, triploblastic, coelomate animals with organ-system level of organisation.

## 21.2 Chordates Versus Non-Chordates

| Feature               | Chordates                          | Non-Chordates                  |
| --------------------- | ---------------------------------- | ------------------------------ |
| Notochord             | Present at least during some stage | Absent                         |
| Nerve cord            | Dorsal, hollow, single             | Usually ventral, solid, double |
| Pharyngeal gill slits | Present at least during some stage | Absent                         |
| Heart                 | Ventral                            | Dorsal if present              |
| Post-anal tail        | Present at least in some stage     | Absent                         |

## 21.3 Subphyla Of Chordata

Phylum Chordata is divided into:

1. Urochordata or Tunicata
2. Cephalochordata
3. Vertebrata

Urochordata and Cephalochordata are often called protochordates. They are exclusively marine.

## 21.4 Urochordata

In Urochordata, the notochord is present only in the larval tail.

Examples:

- Ascidia;
- Salpa;
- Doliolum.

## 21.5 Cephalochordata

In Cephalochordata, the notochord extends from head to tail region and persists throughout life.

Example:

- Branchiostoma, also called Amphioxus or Lancelet.

## 22. Vertebrata

Vertebrates possess a notochord during the embryonic period.

In adults, the notochord is replaced by a cartilaginous or bony vertebral column.

Therefore:

- all vertebrates are chordates;
- all chordates are not vertebrates.

## 22.1 General Features Of Vertebrates

Vertebrates have:

- vertebral column in adult stage;
- ventral muscular heart;
- kidneys for excretion and osmoregulation;
- paired appendages as fins or limbs;
- closed circulatory system.

## 22.2 Major Vertebrate Groups

| Division Or Superclass | Group        | Main Idea                    |
| ---------------------- | ------------ | ---------------------------- |
| Agnatha                | Cyclostomata | Jawless vertebrates          |
| Gnathostomata          | Pisces       | Jawed vertebrates with fins  |
| Gnathostomata          | Tetrapoda    | Jawed vertebrates with limbs |

Pisces includes:

- Chondrichthyes;
- Osteichthyes.

Tetrapoda includes:

- Amphibia;
- Reptilia;
- Aves;
- Mammalia.

## 23. Class Cyclostomata

Cyclostomes are jawless vertebrates.

## 23.1 Main Features Of Cyclostomata

| Feature         | Description                                    |
| --------------- | ---------------------------------------------- |
| Habit           | Ectoparasites on fishes                        |
| Body            | Elongated                                      |
| Gill slits      | 6 to 15 pairs                                  |
| Mouth           | Sucking and circular                           |
| Jaws            | Absent                                         |
| Scales          | Absent                                         |
| Paired fins     | Absent                                         |
| Skeleton        | Cartilaginous cranium and vertebral column     |
| Circulation     | Closed                                         |
| Habitat pattern | Marine, but migrate to freshwater for spawning |

After spawning, adults die within a few days. Larvae return to the ocean after metamorphosis.

## 23.2 Examples Of Cyclostomata

Examples:

- Petromyzon;
- Myxine.

## 24. Class Chondrichthyes

Chondrichthyes are cartilaginous fishes.

## 24.1 Main Features Of Chondrichthyes

| Feature          | Description                    |
| ---------------- | ------------------------------ |
| Habitat          | Marine                         |
| Body             | Streamlined                    |
| Endoskeleton     | Cartilaginous                  |
| Mouth            | Ventral                        |
| Notochord        | Persistent throughout life     |
| Gill slits       | Separate and without operculum |
| Scales           | Placoid scales                 |
| Teeth            | Modified placoid scales        |
| Feeding habit    | Predaceous                     |
| Air bladder      | Absent                         |
| Heart            | Two-chambered                  |
| Body temperature | Poikilothermous                |
| Sexes            | Separate                       |
| Male structure   | Claspers on pelvic fins        |
| Fertilisation    | Internal                       |
| Development      | Many are viviparous            |

## 24.2 Special Features

Some chondrichthyans have electric organs, such as Torpedo.

Some have poison sting, such as Trygon.

Because air bladder is absent, many must swim constantly to avoid sinking.

## 24.3 Examples Of Chondrichthyes

Examples:

- Scoliodon;
- Pristis;
- Carcharodon;
- Trygon.

## 25. Class Osteichthyes

Osteichthyes are bony fishes.

## 25.1 Main Features Of Osteichthyes

| Feature          | Description           |
| ---------------- | --------------------- |
| Habitat          | Marine and freshwater |
| Body             | Streamlined           |
| Endoskeleton     | Bony                  |
| Mouth            | Usually terminal      |
| Gills            | Four pairs            |
| Operculum        | Present               |
| Scales           | Cycloid or ctenoid    |
| Air bladder      | Present               |
| Heart            | Two-chambered         |
| Body temperature | Poikilothermous       |
| Sexes            | Separate              |
| Fertilisation    | Usually external      |
| Reproduction     | Mostly oviparous      |
| Development      | Direct                |

## 25.2 Importance Of Air Bladder

Air bladder regulates buoyancy.

It helps the fish remain at a desired depth without continuous swimming. This saves energy and prevents sinking.

## 25.3 Examples Of Osteichthyes

| Habitat Or Use | Examples               |
| -------------- | ---------------------- |
| Marine         | Exocoetus, Hippocampus |
| Freshwater     | Labeo, Catla, Clarias  |
| Aquarium       | Betta, Pterophyllum    |

## 26. Class Amphibia

Amphibians can live in aquatic as well as terrestrial habitats.

## 26.1 Main Features Of Amphibia

| Feature          | Description              |
| ---------------- | ------------------------ |
| Habitat          | Aquatic and terrestrial  |
| Limbs            | Usually two pairs        |
| Body regions     | Head and trunk           |
| Tail             | Present in some          |
| Skin             | Moist and without scales |
| Eyes             | Eyelids present          |
| Ear              | Tympanum represents ear  |
| Cloaca           | Present                  |
| Respiration      | Gills, lungs, and skin   |
| Heart            | Three-chambered          |
| Body temperature | Poikilothermous          |
| Sexes            | Separate                 |
| Fertilisation    | External                 |
| Reproduction     | Oviparous                |
| Development      | Indirect                 |

## 26.2 Examples Of Amphibia

Examples:

- Bufo;
- Rana;
- Hyla;
- Salamandra;
- Ichthyophis.

## 27. Class Reptilia

Reptiles are mostly terrestrial animals with dry skin.

## 27.1 Main Features Of Reptilia

| Feature              | Description                                           |
| -------------------- | ----------------------------------------------------- |
| Habitat              | Mostly terrestrial                                    |
| Skin                 | Dry, cornified, with scales or scutes                 |
| External ear opening | Absent                                                |
| Ear                  | Tympanum represents ear                               |
| Limbs                | Two pairs when present                                |
| Heart                | Usually three-chambered; four-chambered in crocodiles |
| Body temperature     | Poikilothermous                                       |
| Scale shedding       | Seen in snakes and lizards                            |
| Sexes                | Separate                                              |
| Fertilisation        | Internal                                              |
| Reproduction         | Oviparous                                             |
| Development          | Direct                                                |

## 27.2 Examples Of Reptilia

Examples:

- Chelone;
- Testudo;
- Chameleon;
- Calotes;
- Crocodilus;
- Alligator;
- Hemidactylus;
- Naja;
- Bangarus;
- Vipera.

## 28. Class Aves

Aves includes birds.

## 28.1 Main Features Of Aves

| Feature          | Description                                          |
| ---------------- | ---------------------------------------------------- |
| Body covering    | Feathers                                             |
| Flight           | Most can fly; some are flightless                    |
| Mouth            | Beak present                                         |
| Forelimbs        | Modified into wings                                  |
| Hind limbs       | Adapted for walking, swimming, perching, or clasping |
| Skin             | Dry, without glands except oil gland at tail base    |
| Endoskeleton     | Fully ossified                                       |
| Long bones       | Hollow with air cavities                             |
| Digestive tract  | Crop and gizzard present                             |
| Heart            | Four-chambered                                       |
| Body temperature | Homoiothermous                                       |
| Respiration      | Lungs with air sacs                                  |
| Sexes            | Separate                                             |
| Fertilisation    | Internal                                             |
| Reproduction     | Oviparous                                            |
| Development      | Direct                                               |

## 28.2 Flight Adaptations In Aves

Birds show several adaptations for flight:

- forelimbs modified into wings;
- feathers for flight and insulation;
- streamlined body;
- hollow air-filled bones;
- powerful flight muscles;
- lungs with air sacs for efficient respiration;
- four-chambered heart for efficient circulation;
- warm-blooded metabolism;
- beak instead of heavy jaws with teeth in modern birds.

## 28.3 Examples Of Aves

Examples:

- Corvus;
- Columba;
- Psittacula;
- Struthio;
- Pavo;
- Aptenodytes;
- Neophron.

## 29. Class Mammalia

Mammals are characterised by mammary glands and hair.

## 29.1 Main Features Of Mammalia

| Feature          | Description                          |
| ---------------- | ------------------------------------ |
| Habitat          | Very diverse                         |
| Mammary glands   | Present                              |
| Limbs            | Usually two pairs, variously adapted |
| Skin             | Hair present                         |
| External ears    | Pinnae present                       |
| Teeth            | Different types present              |
| Heart            | Four-chambered                       |
| Body temperature | Homoiothermous                       |
| Respiration      | Lungs                                |
| Sexes            | Separate                             |
| Fertilisation    | Internal                             |
| Reproduction     | Mostly viviparous                    |
| Development      | Direct                               |

## 29.2 Oviparous Exception

Most mammals are viviparous, but a few are oviparous.

Example:

- Ornithorhynchus.

## 29.3 Examples Of Mammalia

Examples:

- Macropus;
- Pteropus;
- Camelus;
- Macaca;
- Rattus;
- Canis;
- Felis;
- Elephas;
- Equus;
- Delphinus;
- Balaenoptera;
- Panthera tigris;
- Panthera leo.

## 30. Salient Features Of Major Phyla

| Phylum          | Organisation | Symmetry            | Coelom       | Segmentation    | Digestive System   | Distinctive Feature                    |
| --------------- | ------------ | ------------------- | ------------ | --------------- | ------------------ | -------------------------------------- |
| Porifera        | Cellular     | Mostly asymmetrical | Absent       | Absent          | Absent as true gut | Pores, canals, choanocytes             |
| Cnidaria        | Tissue       | Radial              | Absent       | Absent          | Incomplete         | Cnidoblasts                            |
| Ctenophora      | Tissue       | Radial              | Absent       | Absent          | Incomplete         | Comb plates                            |
| Platyhelminthes | Organ        | Bilateral           | Absent       | Absent          | Incomplete         | Flat body, flame cells                 |
| Aschelminthes   | Organ-system | Bilateral           | Pseudocoelom | Absent          | Complete           | Round body, muscular pharynx           |
| Annelida        | Organ-system | Bilateral           | True coelom  | Present         | Complete           | Metamerism, nephridia                  |
| Arthropoda      | Organ-system | Bilateral           | True coelom  | Present         | Complete           | Chitin exoskeleton, jointed appendages |
| Mollusca        | Organ-system | Bilateral           | True coelom  | Absent          | Complete           | Mantle, shell, radula                  |
| Echinodermata   | Organ-system | Adult radial        | True coelom  | Absent          | Complete           | Water vascular system                  |
| Hemichordata    | Organ-system | Bilateral           | True coelom  | Absent          | Complete           | Proboscis, collar, trunk               |
| Chordata        | Organ-system | Bilateral           | True coelom  | Present in many | Complete           | Notochord, dorsal hollow nerve cord    |

## 31. Vertebrate Class Comparison

| Class          | Skeleton        | Heart                             | Respiration             | Temperature             | Fertilisation                | Reproduction        | Key Feature                    |
| -------------- | --------------- | --------------------------------- | ----------------------- | ----------------------- | ---------------------------- | ------------------- | ------------------------------ |
| Cyclostomata   | Cartilaginous   | Not emphasised in detail here     | Gill slits              | Poikilothermous pattern | External in spawning context | Spawn in freshwater | Jawless circular mouth         |
| Chondrichthyes | Cartilaginous   | Two-chambered                     | Gills without operculum | Poikilothermous         | Internal                     | Many viviparous     | Placoid scales, no air bladder |
| Osteichthyes   | Bony            | Two-chambered                     | Gills with operculum    | Poikilothermous         | Usually external             | Mostly oviparous    | Air bladder                    |
| Amphibia       | Bony            | Three-chambered                   | Gills, lungs, skin      | Poikilothermous         | External                     | Oviparous           | Moist skin, cloaca             |
| Reptilia       | Bony            | Usually three; four in crocodiles | Lungs                   | Poikilothermous         | Internal                     | Oviparous           | Dry scales or scutes           |
| Aves           | Bony, pneumatic | Four-chambered                    | Lungs with air sacs     | Homoiothermous          | Internal                     | Oviparous           | Feathers, wings                |
| Mammalia       | Bony            | Four-chambered                    | Lungs                   | Homoiothermous          | Internal                     | Mostly viviparous   | Mammary glands, hair           |

## 32. Common Learner Mistakes

| Mistake                                     | Correction                                                                 |
| ------------------------------------------- | -------------------------------------------------------------------------- |
| Treating all worm-like animals as one group | Flatworms, roundworms, annelids, and hemichordates differ strongly         |
| Saying all radial animals are simple        | Adult echinoderms are radial but have organ-system organisation            |
| Confusing coelom and gut cavity             | Coelom is body cavity lined by mesoderm, not the digestive cavity          |
| Calling pseudocoelom a true coelom          | Pseudocoelom is not completely lined by mesoderm                           |
| Saying all chordates are vertebrates        | Vertebrates are only one subphylum of chordates                            |
| Forgetting that protochordates are marine   | Urochordata and Cephalochordata are exclusively marine                     |
| Calling Hemichordata true chordate          | Hemichordata is treated as a separate non-chordate phylum here             |
| Mixing Chondrichthyes and Osteichthyes      | Cartilaginous fishes lack operculum and air bladder; bony fishes have both |
| Saying all mammals give birth               | A few mammals such as Ornithorhynchus lay eggs                             |
| Saying all birds fly                        | Some birds are flightless                                                  |

## 33. Exercise Answers

## 33.1 Difficulties Without Common Fundamental Features

If common fundamental features are not used, animal classification becomes confusing and unreliable.

The main difficulties would be:

- animals with similar outward appearance but different body plans may be grouped together;
- closely related animals may be separated because of superficial differences;
- newly discovered species would be hard to place systematically;
- evolutionary relationships would remain unclear;
- important features such as coelom, symmetry, germ layers, and notochord would be ignored;
- classification would become based on guesswork rather than biological organisation.

For example, a flatworm, roundworm, and annelid may all look worm-like, but they differ in coelom, segmentation, and organ-system development.

## 33.2 Steps To Classify A Given Specimen

To classify an animal specimen, follow these steps:

1. Observe whether it is multicellular and animal-like.
2. Check the level of organisation: cellular, tissue, organ, or organ-system.
3. Identify symmetry: asymmetrical, radial, or bilateral.
4. Determine germ layers if developmental information is available: diploblastic or triploblastic.
5. Check the body cavity: acoelomate, pseudocoelomate, or coelomate.
6. Look for segmentation.
7. Check for notochord or chordate characters.
8. Observe special structures such as pores, cnidoblasts, comb plates, flame cells, jointed appendages, radula, water vascular system, or vertebral column.
9. Compare the features with known phyla and classes.
10. Use diagnostic examples and keys to confirm the group.

## 33.3 Usefulness Of Coelom In Animal Classification

Coelom is highly useful in classification because it reflects internal body organisation.

Animals can be grouped as:

| Type            | Meaning                                       | Example                                                 |
| --------------- | --------------------------------------------- | ------------------------------------------------------- |
| Acoelomate      | Body cavity absent                            | Platyhelminthes                                         |
| Pseudocoelomate | False body cavity not fully lined by mesoderm | Aschelminthes                                           |
| Coelomate       | True body cavity lined by mesoderm            | Annelida, Arthropoda, Mollusca, Echinodermata, Chordata |

The presence of coelom allows better organ development and separation of gut and body wall. It is therefore an important evolutionary and classification feature.

## 33.4 Distinguish Between Intracellular And Extracellular Digestion

| Feature       | Intracellular Digestion                 | Extracellular Digestion           |
| ------------- | --------------------------------------- | --------------------------------- |
| Site          | Inside cells                            | In a cavity or digestive tract    |
| Complexity    | Simpler                                 | More advanced                     |
| Seen in       | Sponges and part of cnidarian digestion | Cnidarians and higher animals     |
| Food handling | Food particles are engulfed by cells    | Enzymes digest food outside cells |

In sponges, digestion is intracellular. In cnidarians, digestion is both extracellular and intracellular.

## 33.5 Difference Between Direct And Indirect Development

| Feature         | Direct Development            | Indirect Development                             |
| --------------- | ----------------------------- | ------------------------------------------------ |
| Larval stage    | Absent                        | Present                                          |
| Young one       | Resembles adult               | Does not resemble adult                          |
| Metamorphosis   | Not required                  | Required                                         |
| Example context | Many reptiles, birds, mammals | Many amphibians, echinoderms, some invertebrates |

In indirect development, the larva is morphologically distinct from the adult.

## 33.6 Peculiar Features Of Parasitic Platyhelminthes

Parasitic platyhelminthes show features that help them live inside hosts.

Important features include:

- dorsoventrally flattened body;
- hooks and suckers for attachment;
- nutrient absorption through body surface in some forms;
- reduced or modified digestive structures in some parasites;
- flame cells for excretion and osmoregulation;
- hermaphroditic condition in many forms;
- internal fertilisation;
- high reproductive output;
- complex development with larval stages.

Examples include Taenia and Fasciola.

## 33.7 Reasons For Arthropods Being The Largest Animal Group

Arthropods form the largest animal phylum due to several successful features:

- jointed appendages for walking, feeding, sensing, and mating;
- chitinous exoskeleton for protection and support;
- segmentation with specialised body regions;
- diverse respiratory organs for different habitats;
- well-developed sense organs;
- open circulation supporting active lifestyles;
- high reproductive capacity;
- metamorphosis in many insects, reducing competition between young and adult stages;
- ability to live in aquatic, terrestrial, and aerial environments.

## 33.8 Water Vascular System Is Characteristic Of Which Group?

Correct answer: Echinodermata.

The water vascular system helps echinoderms in locomotion, food capture, transport of food, and respiration.

## 33.9 Justify: All Vertebrates Are Chordates But All Chordates Are Not Vertebrates

All vertebrates show chordate characters at least during embryonic life, such as notochord, dorsal hollow nerve cord, and pharyngeal gill slits. Therefore, all vertebrates are chordates.

However, not all chordates develop a vertebral column. Urochordates and cephalochordates are chordates but not vertebrates.

In vertebrates, the embryonic notochord is replaced by a cartilaginous or bony vertebral column in the adult.

## 33.10 Importance Of Air Bladder In Pisces

Air bladder is important in bony fishes because it regulates buoyancy.

It helps the fish stay at a particular depth without continuous swimming. This saves energy and prevents sinking.

Cartilaginous fishes lack an air bladder, so many must swim constantly to avoid sinking.

## 33.11 Modifications In Birds That Help In Flight

Birds show several flight adaptations:

- forelimbs modified into wings;
- feathers present;
- streamlined body;
- hollow bones with air cavities;
- powerful flight muscles;
- lungs with air sacs;
- four-chambered heart;
- warm-blooded metabolism;
- beak instead of heavy toothed jaws in modern birds;
- hind limbs adapted for landing, walking, swimming, or perching.

## 33.12 Can The Number Of Eggs Or Young Ones Produced By Oviparous And Viviparous Mothers Be Equal?

Usually, the number is not equal.

Oviparous animals often produce many eggs because the eggs develop outside the mother's body and face greater risk from predators and environmental conditions.

Viviparous animals usually produce fewer young ones because the young develop inside the mother's body, receive better protection, and require more parental investment.

So, while exceptions can occur, in general oviparous animals produce more offspring than viviparous animals.

## 33.13 Segmentation In The Body Is First Observed In Which Group?

Correct answer: Annelida.

Annelids show metameric segmentation, where the body is divided into repeated segments.

## 33.14 Match The Following

| Item        | Correct Match | Group                           |
| ----------- | ------------- | ------------------------------- |
| Operculum   | viii          | Osteichthyes                    |
| Parapodia   | v             | Annelida                        |
| Scales      | iv            | Reptilia                        |
| Comb plates | i             | Ctenophora                      |
| Radula      | ii            | Mollusca                        |
| Hairs       | vii           | Mammalia                        |
| Choanocytes | iii           | Porifera                        |
| Gill slits  | vi            | Cyclostomata and Chondrichthyes |

## 33.15 List Some Animals Parasitic On Human Beings

Examples of animals or animal-like parasites found in or on humans include:

| Parasite    | Group Or Type   | Disease Or Condition |
| ----------- | --------------- | -------------------- |
| Taenia      | Platyhelminthes | Tapeworm infection   |
| Ascaris     | Aschelminthes   | Ascariasis           |
| Wuchereria  | Aschelminthes   | Filariasis           |
| Ancylostoma | Aschelminthes   | Hookworm infection   |
| Enterobius  | Aschelminthes   | Pinworm infection    |
| Schistosoma | Platyhelminthes | Schistosomiasis      |
| Pediculus   | Arthropoda      | Lice infestation     |

## 34. Exam-Oriented Short Answers

## 34.1 Why Are Sponges Considered Primitive Multicellular Animals?

Sponges are multicellular but show only cellular level of organisation. Their cells perform some division of labour, but true tissues and organs are absent.

## 34.2 Why Are Cnidarians Named So?

Cnidarians have special stinging cells called cnidoblasts or cnidocytes. These contain nematocysts used for anchorage, defence, and prey capture.

## 34.3 Why Are Ctenophores Easy To Identify?

Ctenophores have eight external rows of ciliated comb plates used for locomotion. They also show well-marked bioluminescence.

## 34.4 Why Are Flatworms Called Acoelomates?

Flatworms lack a body cavity between body wall and gut wall. Therefore, they are called acoelomates.

## 34.5 Why Are Roundworms Pseudocoelomates?

Roundworms have a body cavity, but it is not completely lined by mesoderm. Therefore, the cavity is called pseudocoelom.

## 34.6 Why Are Annelids More Advanced Than Roundworms?

Annelids have a true coelom, metameric segmentation, closed circulatory system, and nephridia. Roundworms are pseudocoelomate and unsegmented.

## 34.7 What Is The Key Feature Of Arthropoda?

The key feature is jointed appendages. Arthropods also have a chitinous exoskeleton and segmented body.

## 34.8 What Is The Key Feature Of Mollusca?

Molluscs usually have a soft unsegmented body, mantle, mantle cavity, muscular foot, and radula.

## 34.9 What Is The Key Feature Of Echinodermata?

The key feature is the water vascular system.

## 34.10 Why Is Hemichordata Not Placed Under Chordata Here?

Hemichordates have a stomochord, not a true notochord. Therefore, they are treated as a separate non-chordate phylum in this chapter.

## 35. Practice Questions

## 35.1 Very Short Answer Questions

1. Name the level of organisation in Porifera.
2. What are choanocytes?
3. Name the stinging cells of cnidarians.
4. What are comb plates?
5. Which phylum has flame cells?
6. Which phylum includes roundworms?
7. What is metamerism?
8. Which phylum has jointed appendages?
9. Name the feeding organ of molluscs.
10. Which phylum has water vascular system?
11. What is stomochord?
12. List three chordate characters.
13. What is a protochordate?
14. Which vertebrate class lacks jaws?
15. Which fishes have cartilaginous endoskeleton?
16. Which fishes have operculum?
17. What is the heart chamber count in amphibians?
18. Which vertebrate class has feathers?
19. What are mammary glands?
20. What does viviparous mean?

## 35.2 Short Answer Questions

1. Compare radial and bilateral symmetry.
2. Compare diploblastic and triploblastic animals.
3. Explain acoelomate, pseudocoelomate, and coelomate conditions.
4. Write the main features of Porifera.
5. Explain metagenesis in cnidarians.
6. Compare Platyhelminthes and Aschelminthes.
7. Write the main features of Annelida.
8. Explain why Arthropoda is successful.
9. Describe mantle and radula in Mollusca.
10. Explain the water vascular system.
11. Compare chordates and non-chordates.
12. Explain why all chordates are not vertebrates.
13. Compare Chondrichthyes and Osteichthyes.
14. Write flight adaptations in birds.
15. Write the main features of mammals.

## 35.3 Long Answer Questions

1. Explain the basis of classification in Animal Kingdom.
2. Describe the non-chordate phyla from Porifera to Echinodermata.
3. Describe Chordata and compare its subphyla.
4. Describe vertebrate classification from Cyclostomata to Mammalia.
5. Compare major animal phyla using organisation, symmetry, coelom, segmentation, and distinctive characters.

## 36. Rapid Revision Sheet

| Topic           | One-Line Revision                               |
| --------------- | ----------------------------------------------- |
| Porifera        | Pores, canal system, choanocytes                |
| Cnidaria        | Cnidoblasts, radial symmetry, diploblastic      |
| Ctenophora      | Comb plates, marine, bioluminescence            |
| Platyhelminthes | Flat, acoelomate, flame cells                   |
| Aschelminthes   | Round, pseudocoelomate, complete gut            |
| Annelida        | True coelom, metameric segmentation             |
| Arthropoda      | Jointed appendages, chitin exoskeleton          |
| Mollusca        | Mantle, shell, radula                           |
| Echinodermata   | Water vascular system, adult radial symmetry    |
| Hemichordata    | Proboscis, collar, trunk, stomochord            |
| Chordata        | Notochord, dorsal hollow nerve cord, gill slits |
| Cyclostomata    | Jawless vertebrates                             |
| Chondrichthyes  | Cartilaginous fishes, no air bladder            |
| Osteichthyes    | Bony fishes, operculum, air bladder             |
| Amphibia        | Moist skin, three-chambered heart               |
| Reptilia        | Dry scales, internal fertilisation              |
| Aves            | Feathers, wings, pneumatic bones                |
| Mammalia        | Mammary glands, hair, mostly viviparous         |

## 37. Raw File Reference

| Item             | Path                                             |
| ---------------- | ------------------------------------------------ |
| Raw input        | `data/class_11/biology/chapter_04.md`            |
| Formatted output | `data_learner_md/class_11/biology/chapter_04.md` |
