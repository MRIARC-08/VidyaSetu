...
export default function MainLayout({ children }: PropsWithChildren) {
  ...
  const elements = [
    ...
    },
    {
      ...
      svg: (
        <svg width="32" height="26" viewBox="0 0 18 16" fill="currentColor">
  ...
  const activeLink = elements
    .filter(
      (element) => path === element.link || path.startsWith(`${element.link}/`)
    )
    .sort((a, b) => b.link.length - a.link.length)[0]?.link;

  return (
    <div className="flex w-screen bg-background text-foreground">
      <div className="flex min-h-screen w-15 flex-col gap-4 bg-accent/40 pt-8">
        <div className="fixed flex w-15 flex-col items-center bg-accent/40">
          {elements.map((val) => (
            <Link
              key={val.name}
              href={val.link}
              aria-label={val.name}
              className={`flex cursor-pointer items-center justify-center p-4 text-foreground transition-all ${
                activeLink === val.link
