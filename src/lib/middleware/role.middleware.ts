 ...
 */
export function requireExactRole(...roles: string[]): (auth: AuthContext) => void {
  return (auth: AuthContext): void => {
 ...
 */
export function hasMinimumRole(auth: AuthContext, minimumRole: string): boolean {
  const userLevel = ROLE_HIERARCHY[auth.role];
